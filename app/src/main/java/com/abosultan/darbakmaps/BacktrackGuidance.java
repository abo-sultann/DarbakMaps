...
public final class BacktrackGuidance {
    ...
    public static void update(Activity a,Location fix){
        ...
        GeoPoint target=navigator.update(fix.getLatitude(),fix.getLongitude());if(target==null){
            NavigationGuidance.noFix(a);
            TextView gapText=a.findViewById(R.id.nav_detail);
            if(gapText!=null)gapText.setText("انتهى المقطع المسجل؛ لا يوجد أثر متصل للمتابعة");
            return;
        }
