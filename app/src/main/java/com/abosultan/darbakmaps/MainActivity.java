...
public final class MainActivity extends Activity implements LocationController.Callback {
    ...
    private void confirmRecommendedMapDownload() {
        showImmersive(new AlertDialog.Builder(this)
                ...
                .setMessage("التنزيل المباشر غير متاح حاليًا. يمكنك إضافة ملف خريطة دربك المعتمد من USB أو الذاكرة، وستعمل الخريطة بدون إنترنت.")
                .setNegativeButton("إلغاء", null)
                .setPositiveButton("إضافة ملف الخريطة", (dialog, which) -> chooseMapFile())
                .create());
    }
    ...
    private void showSavedHub() {
        ...
        showImmersive(new AlertDialog.Builder(this)
                ...
                .setItems(items, (dialog, which) -> {
                    if (which == 0) {
                        try { showPlaces(placeRepository.all(), "المواقع المحفوظة"); } catch(RuntimeException error) { toast("تعذر قراءة المحفوظات؛ بياناتك الأصلية محفوظة"); }
                    } else if (which == 1) {
                        showNearbyPlaces();
                    } else {
                        showTracks();
        ...
        } catch (RuntimeException error) { toast("تعذر قراءة المحفوظات؛ بياناتك الأصلية محفوظة"); }
    }
