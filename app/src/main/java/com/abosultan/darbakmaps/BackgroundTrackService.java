...
public final class BackgroundTrackService extends Service implements LocationListener {
    ...
    private LocationManager manager;private Location last;private boolean listening, ending;private int latestStartId;private long revision=-1;
    ...
    public static void ensureRunning(Context c){if(finishing(c)||MapUiPreferences.backgroundTrackEnabled(c))launch(c,new Intent(c,BackgroundTrackService.class).setAction(finishing(c)?STOP:START));}
    ...
    @Override public void onCreate(){super.onCreate();manager=(LocationManager)getSystemService(LOCATION_SERVICE);startForeground(2306,notification("بانتظار GPS"));}
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        if((intent!=null&&STOP.equals(intent.getAction()))||!MapUiPreferences.backgroundTrackEnabled(this)){
            ...
            writer.execute(()->{
                ...
                final String resultMessage=message;final boolean resultSuccess=success;
                new Handler(Looper.getMainLooper()).post(()->{
                    state(this).edit().putBoolean("finishing",false).putString("status",resultMessage).commit();
                    sendBroadcast(new Intent(RESULT).setPackage(getPackageName()).putExtra("message",resultMessage).putExtra("success",resultSuccess));
                    stopForeground(true);stopSelf(latestStartId);
                });
    ...
    @Override public void onLocationChanged(Location input){
        ...
        writer.execute(()->{
            if(TrackSessionState.isPaused(this)||currentRevision!=TrackSessionState.segmentRevision(this)||!LocationController.isUsable(fix))return;
