...
public final class TrackNavigator {
    private final List<GeoPoint> points;private int cursor=-1;
    ...
    public GeoPoint update(double lat,double lon){
        ...
        if(cursor<0){double best=Double.MAX_VALUE;int index=points.size()-2;
        ...
        while(cursor>0&&distance(lat,lon,points.get(cursor).latitude,points.get(cursor).longitude)<30){cursor--;}
    ...
    public boolean crossesGap(){return cursor>=0&&cursor+1<points.size()&&points.get(cursor+1).segmentStart;}
