...
final class SettingsDialog {
 static void show(Context c){
  ...
  TextView importMap=DarbakUi.action(c,map.isFile()?"استبدال الخريطة":"إضافة خريطة");importMap.setOnClickListener(v->{if(holder[0]!=null)holder[0].dismiss();if(c instanceof MainActivity)((MainActivity)c).pickOfflineMap();else Toast.makeText(c,"تعذر فتح اختيار الخريطة",Toast.LENGTH_SHORT).show();});box.addView(importMap,rowParams(c,6));addToggle(c,box,s,MapUiSettings.POI,"إظهار المعالم و POI");addToggle(c,box,s,MapUiSettings.ROTATE_MAP,"تدوير الخريطة حسب اتجاه الحركة");
  ...
  addSection(c,box,"المواقع");addToggle(c,box,s,MapUiSettings.SAVED_MARKERS,"إظهار المواقع المحفوظة على الخريطة");
  addSection(c,box,"المسارات والتنبيهات");addSessionToggle(c,box,session,true,"تسجيل المسار تلقائيًا");addToggle(c,box,s,MapUiSettings.OFF_TRACK_ALERT,"تنبيه الابتعاد عن المسار");
