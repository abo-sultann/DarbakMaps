...
public final class HomeScreen extends FrameLayout {
    private static final long AUTO_HIDE_MS=6500L, LONG_PRESS_MS=700L;
    ...
    private void build(Context c){
        ...
        longPressSave=()->{if(!moved){longPressed=true;pickerAt(c,map,downX,downY);}};
    ...
    private static void pickerAt(Context c,OfflineMapView map,float x,float y){String[] l={"طير سمان","ماء","مخيم","وقود","أخرى"},v={"summan","water","camp","fuel","other"};DarbakDialog.menu(c,"حفظ هذه النقطة كـ",l,w->{boolean ok=map.saveMapPlace(x,y,v[w]);Toast.makeText(c,ok?"تم حفظ النقطة — "+l[w]:"تعذر تحديد النقطة على الخريطة",Toast.LENGTH_SHORT).show();});}
