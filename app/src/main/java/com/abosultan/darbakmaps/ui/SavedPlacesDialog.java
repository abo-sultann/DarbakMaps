...
final class SavedPlacesDialog {
    static void show(Context c, OfflineMapView map) {
        ...
        if(!fix.valid){Toast.makeText(c,"بانتظار إشارة GPS لترتيب المواقع حسب الأقرب",Toast.LENGTH_SHORT).show();return;}
        SqlitePlaceRepository repo=new SqlitePlaceRepository(c);
        final List<Place> places;
        try { places=repo.nearest(fix.latitude,fix.longitude,null,100); } finally { repo.close(); }
        ...
        for(int i=0;i<places.size();i++){
            Place p=places.get(i); double meters=PlaceMath.distanceMeters(fix.latitude,fix.longitude,p.latitude,p.longitude);
        ...
        new AlertDialog.Builder(c).setTitle("المواقع المحفوظة — الأقرب أولاً").setItems(rows,(d,which)->{
            Place p=places.get(which); map.focusPlace(p.latitude,p.longitude); Toast.makeText(c,"تم إظهار الموقع على الخريطة",Toast.LENGTH_SHORT).show();
