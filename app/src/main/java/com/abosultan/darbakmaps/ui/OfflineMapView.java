...
public final class OfflineMapView extends FrameLayout {
    ...
    void showPlaceActions(final Place place){if(place==null)return;focusPlace(place.latitude,place.longitude);LocationSnapshot fix=LiveLocationStore.latest();StringBuilder m=new StringBuilder(categoryLabel(place.category));if(fix.valid){double meters=PlaceMath.distanceMeters(fix.latitude,fix.longitude,place.latitude,place.longitude);double targetBearing=bearingDegrees(fix.latitude,fix.longitude,place.latitude,place.longitude);boolean headingReliable=fix.speedKmh>=MIN_HEADING_SPEED_KMH&&validBearing(fix.bearing);m.append("\nالمسافة: ").append(formatDistance(meters));if(headingReliable)m.append("\nالاتجاه حسب السيارة: ").append(directionArrow(relativeBearing(targetBearing,fix.bearing)));else m.append("\nالاتجاه: ").append(cardinalDirection(targetBearing));}else m.append("\nGPS غير متاح لحساب المسافة والاتجاه");DarbakDialog.infoActions(getContext(),"الموقع المحفوظ",m.toString(),new String[]{"توجيه مباشر","إلغاء التوجيه"},which->{if(which==0)startGuidance(place);else stopGuidance();});}
    private void startGuidance(Place p){backtrackNavigator=null;guidanceTarget=p;LocationSnapshot f=LiveLocationStore.latest();if(f.valid){LatLong current=new LatLong(f.latitude,f.longitude);double meters=PlaceMath.distanceMeters(f.latitude,f.longitude,p.latitude,p.longitude);if(meters<=DIRECT_GUIDANCE_REACHED_METERS){guidanceTarget=null;if(guidanceLine!=null)guidanceLine.clear();Toast.makeText(getContext(),"أنت عند الموقع المحفوظ",Toast.LENGTH_SHORT).show();return;}updateGuidance(current);centerOn(current,(byte)15);}else focusPlace(p.latitude,p.longitude);Toast.makeText(getContext(),"تم بدء التوجيه المباشر للموقع",Toast.LENGTH_SHORT).show();}
    ...
    private void updateGuidance(LatLong current){if(guidanceLine==null||guidanceTarget==null||current==null)return;double meters=PlaceMath.distanceMeters(current.latitude,current.longitude,guidanceTarget.latitude,guidanceTarget.longitude);if(meters<=DIRECT_GUIDANCE_REACHED_METERS){guidanceTarget=null;guidanceLine.clear();if(mapView!=null)mapView.getLayerManager().redrawLayers();Toast.makeText(getContext(),"وصلت إلى الموقع المحفوظ",Toast.LENGTH_SHORT).show();return;}guidanceLine.clear();guidanceLine.addPoint(current);guidanceLine.addPoint(new LatLong(guidanceTarget.latitude,guidanceTarget.longitude));if(mapView!=null)mapView.getLayerManager().redrawLayers();}
    ...
    private void updateDrivingHeading(LatLong p,LocationSnapshot fix){if(mapView==null||p==null||fix==null)return;mapView.setCenter(p);if(fix.speedKmh>=MIN_HEADING_SPEED_KMH&&validBearing(fix.bearing)){float target=-fix.bearing;if(Float.isNaN(lastMapBearing)||Math.abs(angleDelta(lastMapBearing,target))>=3f){mapView.setScaleX(HEADING_OVERSCAN);mapView.setScaleY(HEADING_OVERSCAN);mapView.setRotation(target);lastMapBearing=target;}}}
    ...
    private void clearLiveGpsVisuals(){if(mapView==null)return;mapView.setRotation(0f);mapView.setScaleX(1f);mapView.setScaleY(1f);lastMapBearing=Float.NaN;boolean changed=false;if(vehicleMarker!=null){mapView.getLayerManager().getLayers().remove(vehicleMarker);vehicleMarker.onDestroy();vehicleMarker=null;lastVehicleBearing=Float.NaN;changed=true;}if(guidanceLine!=null){guidanceLine.clear();changed=true;}if(changed)mapView.getLayerManager().redrawLayers();}
