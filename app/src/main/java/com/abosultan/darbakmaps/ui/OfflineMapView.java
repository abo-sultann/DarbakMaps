...
public final class OfflineMapView extends FrameLayout {
    ...
    public LatLong mapPoint(float x,float y){
        if(mapView==null)return null;
        try{return mapView.getMapViewProjection().fromPixels(x,y);}catch(RuntimeException e){return null;}
    }
    ...
    private boolean savePlace(double lat,double lon,String category){
        if(placeStore==null||lat<-90d||lat>90d||lon<-180d||lon>180d)return false;
        String safe=category==null||category.length()==0?"other":category;
        try{long id=placeStore.save(new Place(0L,lat,lon,safe,null,null));if(id<=0L)return false;if(mapView!=null)addPlaceMarker(new Place(id,lat,lon,safe,null,null));return true;}catch(RuntimeException e){return false;}
    }
    ...
    void showPlaceActions(final Place place){if(place==null)return;focusPlace(place.latitude,place.longitude);LocationSnapshot fix=LiveLocationStore.latest();StringBuilder m=new StringBuilder(categoryLabel(place.category));if(fix.valid){double meters=PlaceMath.distanceMeters(fix.latitude,fix.longitude,place.latitude,place.longitude);double bearing=bearingDegrees(fix.latitude,fix.longitude,place.latitude,place.longitude);m.append("\nالمسافة: ").append(formatDistance(meters));m.append("\nالاتجاه: ").append(directionArrow(bearing)).append("  ").append(Math.round(bearing)).append("°");}else m.append("\nبانتظار GPS لحساب المسافة والاتجاه");new AlertDialog.Builder(getContext()).setTitle("الموقع المحفوظ").setMessage(m.toString()).setPositiveButton("توجيه مباشر",(d,w)->startGuidance(place)).setNeutralButton("إلغاء التوجيه",(d,w)->stopGuidance()).setNegativeButton("إغلاق",null).show();}
