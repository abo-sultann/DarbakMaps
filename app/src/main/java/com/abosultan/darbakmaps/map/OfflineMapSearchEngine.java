...
public final class OfflineMapSearchEngine {
    ...
    public synchronized List<Result> search(String query, File activeMap,
        ...
        ranked.sort((first, second) -> {
            ...
            float firstDistance = first.result.distanceMeters == null
                    ? Float.MAX_VALUE : first.result.distanceMeters;
            float secondDistance = second.result.distanceMeters == null
                    ? Float.MAX_VALUE : second.result.distanceMeters;
    ...
    private void addRanked(List<RankedResult> output, Result result, String query) {
        ...
        if (score > 0) {
            output.add(new RankedResult(result, score));
        }
    }
    ...
    private TileRef focusTile(File file, Double latitude, Double longitude) {
        ...
        try {
            ...
            double lat = inside(latitude, bounds.minLatitude, bounds.maxLatitude)
                    ? latitude : center.latitude;
            double lon = inside(longitude, bounds.minLongitude, bounds.maxLongitude)
                    ? longitude : center.longitude;
            return new TileRef(
                    MercatorProjection.longitudeToTileX(lon, zoom),
    ...
    private List<Result> buildIndex(File file, Double latitude, Double longitude) {
        ...
        try {
            ...
            LatLong center = bounds.getCenterPoint();
            double focusLatitude = inside(latitude, bounds.minLatitude, bounds.maxLatitude)
                    ? latitude : center.latitude;
            double focusLongitude = inside(longitude, bounds.minLongitude, bounds.maxLongitude)
                    ? longitude : center.longitude;
            ...
            for (TileRef tileRef : tiles) {
                ...
                for (Way way : data.ways) {
                    ...
                    if (position == null && way.latLongs.length > 0 && way.latLongs[0].length > 0) {
                        position = way.latLongs[0][0];
                    }
                    if (position != null) {
                        addItem(output, way.tags, position, "اسم على الخريطة");
                    }
                }
            }
        } catch (Exception ignored) {
    ...
    private void addItem(Map<String, Result> output, List<Tag> tags,
                         LatLong position, String fallbackSource) {
        String source = classify(tags, fallbackSource);
        String name = firstNotBlank(
                tagValue(tags, "name:ar"),
                tagValue(tags, "name"),
                tagValue(tags, "name:en")
        );
        if (name == null && GENERIC_SERVICE_NAMES.contains(source)) {
            name = source;
        }
        if (name == null) {
            return;
    ...
    private String classify(List<Tag> tags, String fallback) {
        ...
        if (oneOf(amenity, "hospital", "clinic", "doctors")
                || oneOf(healthcare, "hospital", "clinic", "doctor")) return "مستشفى/عيادة";
    ...
    private String tagValue(List<Tag> tags, String key) {
        for (Tag tag : tags) {
            if (key.equalsIgnoreCase(tag.key)) {
                return tag.value;
            }
        }
        return null;
    ...
    private boolean inside(Double value, double min, double max) {
    ...
    public static final class Result {
        ...
        Result(String id, String name, double latitude, double longitude,
               String source, boolean saved, Float distanceMeters) {
            this.id = id;
            this.name = name;
        ...
        Result withDistance(Float distance) {
            return new Result(id, name, latitude, longitude, source, saved, distance);
        }
    }
    ...
    private static final class RankedResult {
        ...
        RankedResult(Result result, int score) {
            this.result = result;
            this.score = score;
        }
    }
