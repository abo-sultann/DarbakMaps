...
public class TrackJournalTest {
 ...
 @Test public void emptyJournalCannotReplaceExistingTrack()throws Exception{
  File dir=Files.createTempDirectory("empty-track").toFile(),source=new File(dir,"active.csv"),target=new File(dir,"saved.gpx");
  source.createNewFile();
  try{TrackJournal.export(source,target,"x");fail();}catch(IOException expected){}
  assertTrue(source.exists());assertFalse(target.exists());
 }
 @Test public void missingInputCannotPublishPartialGpx()throws Exception{
  File dir=Files.createTempDirectory("missing-track").toFile(),source=new File(dir,"missing.csv"),target=new File(dir,"saved.gpx");
  try{TrackJournal.export(source,target,"x");fail();}catch(IOException expected){}
  assertFalse(target.exists());assertFalse(new File(target.getAbsolutePath()+".pending").exists());
 }
}
